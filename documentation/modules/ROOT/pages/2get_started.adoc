...
After you have deployed the Java service, you would have noticed that it has triggered a new Kubernetes deployment something like `greeter-deployment-00002`.  Wondering the reason for it, lets compare the two `service.yaml` that were deployed:
...
[NOTE]
====
Each revision will trigger new deployments, and since the service is configured with **runLatest** it always serves the latest revision.  There is also another type of service deployed which can run with xref:3.revisions.adoc[Revisions].
====